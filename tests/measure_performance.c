        ...
        uint32_t request_id, struct lyd_node **parent, void *private_data)
{
    ...
    for (size_t i = 0; i < if_count; i++) {
        ...
        free(xpath);
        assert_int_equal(LY_SUCCESS, lyd_new_term(list, NULL, "oper-status", "up", NULL));

        assert_int_equal(LY_SUCCESS, lyd_new_inner(list, NULL, "statistics", &stats));
        assert_int_equal(LY_SUCCESS, lyd_new_term(stats, NULL, "in-octets", "456213", NULL));
        assert_int_equal(LY_SUCCESS, lyd_new_term(stats, NULL, "in-unicast-pkts", "45213", NULL));
        assert_int_equal(LY_SUCCESS, lyd_new_term(stats, NULL, "in-broadcast-pkts", "4213", NULL));
    }
...
perf_libyang_get_node(void **state, int op_num, int *items)
{
    ...
    assert_int_equal(LY_SUCCESS, lyd_parse_data_path(ctx, EXAMPLE_MODULE_DATA_FILE_NAME, LYD_LYB,
            LYD_PARSE_NO_STATE | LYD_PARSE_STRICT, 0, &root));
