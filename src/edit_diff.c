...
/**
 * @brief Find PID and conn-ptr metadata of a diff node or its parents.
 *
...
sr_diff_find_pid_conn(struct lyd_node *diff, pid_t *pid, void **conn_ptr, int *meta_own)
{
    sr_error_info_t *err_info = NULL;
    struct lyd_node *parent;
    struct lyd_meta *meta, *pid_meta = NULL, *conn_meta = NULL;
    ...
    for (parent = diff; parent; parent = lyd_parent(parent)) {
        LY_LIST_FOR(parent->meta, meta) {
            if (!pid_meta && !strcmp(meta->name, "pid") && !strcmp(meta->annotation->module->name, SR_YANG_MOD)) {
                pid_meta = meta;
                if (meta_own && (parent == diff)) {
                    *meta_own = 1;
                }
            }
            if (!conn_meta && !strcmp(meta->name, "conn-ptr") && !strcmp(meta->annotation->module->name, SR_YANG_MOD)) {
                conn_meta = meta;
            }
        }

        if ((!pid && !conn_ptr) || (pid_meta && conn_meta)) {
            /* we found everything */
            if (pid) {
                *pid = (pid_t)pid_meta->value.uint32;
            }
            if (conn_ptr) {
                *conn_ptr = (void *)((uintptr_t)conn_meta->value.uint64);
            }
            if (meta_own && (parent == diff) && pid_meta) {
                *meta_own = 1;
            }
            break;
        }

        if ((pid_meta && !conn_meta) || (!pid_meta && conn_meta)) {
...
/**
 * @brief Check (inherited) pid and conn-ptr meadata of a diff node. Replace if not matching this connection and PID.
 *
 ...
 * @param[in] cur_meta_own Whether \p old_pid and \p old_conn_ptr are owned or inherited.
 ...
 */
static sr_error_info_t *
sr_diff_check_pid_conn(struct lyd_node *diff_node, pid_t cur_pid, void *cur_conn_ptr, int cur_meta_own, void *conn_ptr,
        int keep_cur_child)
{
    sr_error_info_t *err_info = NULL;
    ...
    if ((!cur_pid && !cur_conn_ptr) || (cur_pid != getpid()) || (cur_conn_ptr != conn_ptr)) {
        if (cur_meta_own) {
            ...
            sr_edit_del_meta(diff_node, "pid");
            sr_edit_del_meta(diff_node, "conn-ptr");
        }

        /* add attrs of the new connection */
        sprintf(pid_str, "%ld", (long)getpid());
        if (lyd_new_meta(diff_node, NULL, SR_YANG_MOD ":pid", pid_str, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
            return err_info;
        }
        sprintf(conn_str, "%" PRIuPTR, (uintptr_t)conn_ptr);
        if (lyd_new_meta(diff_node, NULL, SR_YANG_MOD ":conn-ptr", conn_str, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
            return err_info;
        }
        ...
        LY_LIST_FOR(lyd_child_no_keys(diff_node), child) {
            if ((err_info = sr_diff_find_pid_conn(child, NULL, NULL, &meta_own))) {
                return err_info;
            }
            if (!meta_own) {
                if (lyd_new_meta(child, NULL, SR_YANG_MOD ":pid", pid_str, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
                    return err_info;
                }
                if (lyd_new_meta(child, NULL, SR_YANG_MOD ":conn-ptr", conn_str, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
                    return err_info;
                }
            }
...
LY_ERR
sr_lyd_diff_merge_cb(const struct lyd_node *src_node, struct lyd_node *trg_node, void *oper_conn)
{
    sr_error_info_t *err_info = NULL;
    pid_t pid;
    void *conn_ptr;
    int meta_own;
    const char *origin, *cur_origin;
    struct lyd_node *child;

    /* learn current PID/conn-ptr */
    if ((err_info = sr_diff_find_pid_conn(trg_node, &pid, &conn_ptr, &meta_own))) {
        goto error;
    }

    /* check PID/conn-ptr of the new node */
    if ((err_info = sr_diff_check_pid_conn(trg_node, pid, conn_ptr, meta_own, oper_conn, 1))) {
        goto error;
    }
        ...
        const char *key_or_value, int dflt_ll_skip, struct lyd_node **match_p, int *val_equal_p)
{
    ...
    if ((op == EDIT_PURGE) && (edit_node->schema->nodetype & (LYS_LIST | LYS_LEAFLIST))) {
        ...
        lyrc = lyd_find_sibling_first(first_node, edit_node, (struct lyd_node **)&match);
        ...
        if (match) {
            ...
            case LYS_LEAFLIST:
                if (dflt_ll_skip && (match->flags & LYD_DEFAULT)) {
                    ...
                    match = NULL;
                } else if (lysc_is_userordered(edit_node->schema)) {
    ...
    *op = parent_op;
    ...
    LY_LIST_FOR(edit_node->meta, meta) {
        ...
        if (!strcmp(meta->name, "operation")) {
            if (!strcmp(meta->annotation->module->name, "ietf-netconf")) {
                switch (val_str[0]) {
                case 'c':
                    assert(!strcmp(val_str, "create"));
                    *op = EDIT_CREATE;
                    break;
                case 'd':
                    assert(!strcmp(val_str, "delete"));
                    *op = EDIT_DELETE;
                    break;
                case 'r':
                    if (!strcmp(val_str, "remove")) {
                        *op = EDIT_REMOVE;
                    } else if (!strcmp(val_str, "replace")) {
                        *op = EDIT_REPLACE;
                    } else {
                        SR_ERRINFO_INT(&err_info);
                        return err_info;
            ...
            } else if (!strcmp(meta->annotation->module->name, SR_YANG_MOD)) {
                switch (val_str[0]) {
                case 'n':
                    assert(!strcmp(val_str, "none"));
                    *op = EDIT_NONE;
                    break;
                case 'e':
                    assert(!strcmp(val_str, "ether"));
                    *op = EDIT_ETHER;
                    break;
                case 'p':
                    assert(!strcmp(val_str, "purge"));
                    *op = EDIT_PURGE;
                    break;
                default:
                    SR_ERRINFO_INT(&err_info);
        ...
        enum insert_val insert, const char *key_or_value)
{
    ...
    if ((insert == INSERT_DEFAULT) || (insert == INSERT_LAST)) {
        ...
        lyd_insert_sibling(*first_node, new_node, NULL);
        return NULL;
...
sr_edit_set_oper(struct lyd_node *edit, const char *op)
{
    ...
    if (lyd_new_meta(edit, NULL, attr_full_name, op, NULL)) {
...
void
sr_edit_del_meta(struct lyd_node *edit, const char *name)
{
    struct lyd_meta *meta;
...
sr_diff_add_meta(struct lyd_node *diff_node, const char *meta_val, const char *prev_meta_val, enum edit_op op)
{
    ...
    case EDIT_NONE:
        ...
        if (diff_node->schema->nodetype & (LYS_LEAF | LYS_LEAFLIST)) {
            if (prev_meta_val && lyd_new_meta(diff_node, NULL, "yang:orig-default", prev_meta_val, NULL)) {
                goto ly_error;
            }
        }
        break;
    case EDIT_REPLACE:
        if (diff_node->schema->nodetype == LYS_LEAF) {
            ...
            if (lyd_new_meta(diff_node, NULL, "yang:orig-value", meta_val, NULL)) {
                goto ly_error;
            }
            if (prev_meta_val && lyd_new_meta(diff_node, NULL, "yang:orig-dflt", prev_meta_val, NULL)) {
                goto ly_error;
            }
            break;
        }
        ...
        if (diff_node->schema->nodetype == LYS_LIST) {
            if (lyd_new_meta(diff_node, NULL, "yang:orig-key", prev_meta_val, NULL)) {
                goto ly_error;
            }
        } else {
            if (lyd_new_meta(diff_node, NULL, "yang:orig-value", prev_meta_val, NULL)) {
                goto ly_error;
            }
        }
        /* fallthrough */
    case EDIT_CREATE:
        if (lysc_is_userordered(diff_node->schema)) {
            ...
            if (diff_node->schema->nodetype == LYS_LIST) {
                if (lyd_new_meta(diff_node, NULL, "yang:key", meta_val, NULL)) {
                    goto ly_error;
                }
            } else {
                if (lyd_new_meta(diff_node, NULL, "yang:value", meta_val, NULL)) {
                    goto ly_error;
                }
            }
        }
...
sr_edit_diff_set_origin(struct lyd_node *node, const char *origin, int overwrite)
{
    ...
    if (cur_origin_own) {
        sr_edit_del_meta(node, "origin");
    }

    /* set correct origin */
    if (lyd_new_meta(node, NULL, "ietf-origin:origin", origin, NULL)) {
        ...
        struct lyd_node *diff_parent, struct lyd_node **diff_root, struct lyd_node **diff_node)
{
    ...
    if ((node_dup->schema->nodetype == LYS_LEAFLIST) && ((struct lysc_node_leaflist *)node_dup->schema)->dflts
        ...
        if (lyd_diff_merge_tree(diff_root, diff_parent, node_dup, NULL, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_CTX(node));
            goto error;
        }
    ...
    if ((op == EDIT_REPLACE) && lysc_is_userordered(diff->schema)) {
        ...
        if (!lyd_compare_meta(orig_meta, meta)) {
            ...
            if (child) {
                ...
                sr_edit_del_meta(diff, "operation");
    ...
    LYD_TREE_DFS_BEGIN(match_subtree, elem) {
        if (lysc_is_userordered(elem->schema)) {
            ...
            if (elem->schema->nodetype == LYS_LIST) {
                if (lyd_new_meta(elem, NULL, "yang:key", sibling_before_val, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_CTX(elem));
                }
            } else {
                if (lyd_new_meta(elem, NULL, "yang:value", sibling_before_val, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_CTX(elem));
                }
            }
            free(sibling_before_val);
        ...
        if (diff && mod_diff) {
            ...
            if (!*diff) {
                *diff = mod_diff;
                ...
                if (lyd_diff_merge_tree(diff, NULL, mod_diff, NULL, NULL)) {
                    return err_info;
                }
                lyd_free_siblings(mod_diff);
            }
...
sr_diff_op(const struct lyd_node *diff_node, enum edit_op *op, const char **key_or_value)
{
    ...
    for (diff_parent = diff_node; diff_parent; diff_parent = lyd_parent(diff_parent)) {
        LY_LIST_FOR(diff_parent->meta, meta) {
            if (!strcmp(meta->name, "operation")) {
                if (!strcmp(meta->annotation->module->name, SR_YANG_MOD)) {
                    assert(!strcmp(meta->value.canonical, "none"));
                    *op = EDIT_NONE;
                    break;
                } else if (!strcmp(meta->annotation->module->name, "ietf-netconf")) {
                    if (!strcmp(meta->value.canonical, "create")) {
                        *op = EDIT_CREATE;
                    } else if (!strcmp(meta->value.canonical, "delete")) {
                        *op = EDIT_DELETE;
                    } else if (!strcmp(meta->value.canonical, "replace")) {
...
sr_diff_update_r(const struct lyd_node *first_node, struct lyd_node *diff_node, struct lyd_node **diff_root)
{
    ...
    case EDIT_CREATE:
        ...
        if (match) {
            ...
            if (sr_edit_find_oper(diff_node, 0, NULL)) {
                sr_edit_del_meta(diff_node, "operation");
            }
            ...
            LY_LIST_FOR(lyd_child_no_keys(diff_node), diff_child) {
                if (!sr_edit_find_oper(diff_child, 0, NULL) && (err_info = sr_edit_set_oper(diff_child, "create"))) {
                    return err_info;
                }
            }
        } else {
...
sr_edit_add_check_same_node_op(sr_session_ctx_t *session, const char *xpath, const char *value, enum edit_op op)
{
    ...
    if (ly_errcode(session->conn->ly_ctx) == LY_EEXIST) {
        ...
        if (!set || (set->count > 1)) {
            ...
            cur_op = sr_edit_find_oper(set->dnodes[0], 1, NULL);
            if (op == cur_op) {
        ...
        const char *origin, int isolate)
{
    ...
    if (lyd_new_path2(isolate ? NULL : session->dt[session->ds].edit, session->conn->ly_ctx, xpath, (void *)value, 0,
            opts, NULL, &node)) {
    ...
    if (!op) {
        ...
        for (parent = lyd_parent(node); parent; node = parent, parent = lyd_parent(parent)) {
            ...
            if (own_oper) {
                ...
                sr_edit_del_meta(parent, "operation");

                if (parent->parent) {
                    /* check whether our operation is superior even to the next defined operation */
                    op = sr_edit_find_oper(lyd_parent(parent), 1, &own_oper);
                    assert(op);
                    next_iter_oper = 1;
                }
    ...
    if (!strcmp(operation, "purge")) {
        ly_mod = ly_ctx_get_module_implemented(session->conn->ly_ctx, SR_YANG_MOD);
    } else {
        ly_mod = ly_ctx_get_module_implemented(session->conn->ly_ctx, "ietf-netconf");
    }
    if (!ly_mod || lyd_new_meta(node, ly_mod, "operation", operation, NULL)) {
        sr_errinfo_new_ly(&err_info, session->conn->ly_ctx);
        goto error;
    }
    if (position) {
        switch (*position) {
        case SR_MOVE_BEFORE:
            if (lyd_new_meta(node, NULL, "yang:insert", "before", NULL)) {
                sr_errinfo_new_ly(&err_info, session->conn->ly_ctx);
                goto error;
            }
            if (lyd_new_meta(node, NULL, node->schema->nodetype == LYS_LIST ? "yang:key" : "yang:value", meta_val, NULL)) {
                sr_errinfo_new_ly(&err_info, session->conn->ly_ctx);
                goto error;
            }
            break;
        case SR_MOVE_AFTER:
            if (lyd_new_meta(node, NULL, "yang:insert", "after", NULL)) {
                sr_errinfo_new_ly(&err_info, session->conn->ly_ctx);
                goto error;
            }
            if (lyd_new_meta(node, NULL, node->schema->nodetype == LYS_LIST ? "yang:key" : "yang:value", meta_val, NULL)) {
            ...
            break;
        case SR_MOVE_FIRST:
            if (lyd_new_meta(node, NULL, "yang:insert", "first", NULL)) {
                sr_errinfo_new_ly(&err_info, session->conn->ly_ctx);
                goto error;
            }
            break;
        case SR_MOVE_LAST:
            if (lyd_new_meta(node, NULL, "yang:insert", "last", NULL)) {
                sr_errinfo_new_ly(&err_info, session->conn->ly_ctx);
                goto error;
            }
            break;
