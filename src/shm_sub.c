    ...
    *update_edit = NULL;
    ...
    while ((mod = sr_modinfo_next_mod(mod, mod_info, mod_info->diff, &aux))) {
        ...
        do {
            ...
            /* parse updated edit */
            if (lyd_parse_data_mem(ly_ctx, shm_sub.addr + sizeof *multi_sub_shm, LYD_LYB, LYD_PARSE_OPAQ | LYD_PARSE_ONLY,
                    0, &edit)) {
                sr_errinfo_new_ly(&err_info, ly_ctx);
        ...
        sr_error_info_t **cb_err_info)
{
    ...
    /* parse returned data */
    if (lyd_parse_data_mem(ly_mod->ctx, shm_sub.addr + sizeof *sub_shm, LYD_LYB, LYD_PARSE_ONLY | LYD_PARSE_STRICT, 0, data)) {
        sr_errinfo_new_ly(&err_info, ly_mod->ctx);
        ...
        uint32_t timeout_ms, uint32_t *request_id, struct lyd_node **output, sr_error_info_t **cb_err_info)
{
    ...
    *output = NULL;
    ...
    do {
        ...
        lyd_free_all(*output);
        ly_in_free(in, 0);
        ly_in_new_memory(shm_sub.addr + sizeof *multi_sub_shm, &in);
        if (lyd_parse_reply(input, in, LYD_LYB, output, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_CTX(input));
...
sr_shmsub_change_listen_has_diff(struct modsub_changesub_s *sub, const struct lyd_node *diff)
{
    ...
    lyd_find_xpath(diff, sub->xpath, &set);
    assert(set);
        ...
        sr_error_info_t **err_info)
{
    ...
    if ((sub_info->event != multi_sub_shm->event) || (sub_info->request_id != multi_sub_shm->request_id)) {
        ...
        if ((sub_info->event == SR_SUB_EV_CHANGE) && (err_code == SR_ERR_OK)
            ...
            if (lyd_diff_reverse_all(tmp_sess->dt[tmp_sess->ds].diff, &abort_diff)) {
                SR_ERRINFO_INT(err_info);
                return 1;
            }
            lyd_free_all(tmp_sess->dt[tmp_sess->ds].diff);
...
sr_shmsub_change_listen_process_module_events(struct modsub_change_s *change_subs, sr_conn_ctx_t *conn)
{
    ...
    /* parse event diff */
    if (lyd_parse_data_mem(conn->ly_ctx, change_subs->sub_shm.addr + sizeof *multi_sub_shm, LYD_LYB,
            LYD_PARSE_OPAQ | LYD_PARSE_STRICT, 0, &diff)) {
        SR_ERRINFO_INT(&err_info);
        goto cleanup_rdunlock;
    }
...
process_event:
    ...
    case SR_SUB_EV_UPDATE:
        if (err_code == SR_ERR_OK) {
            ...
            LY_LIST_FOR(tmp_sess.dt[tmp_sess.ds].edit, iter) {
                if (strcmp(lyd_owner_module(iter)->name, change_subs->module_name)) {
                    /* generate an error */
                    path = lyd_path(iter, LYD_PATH_LOG, NULL, 0);
            ...
            /* print it into LYB */
            if ((err_info = sr_lyd_print_lyb(tmp_sess.dt[tmp_sess.ds].edit, &data, &data_len))) {
                goto cleanup_rdunlock;
            }
        }
        /* fallthrough */
...
sr_shmsub_oper_listen_process_module_events(struct modsub_oper_s *oper_subs, sr_conn_ctx_t *conn)
{
    ...
    for (i = 0; (err_code == SR_ERR_OK) && (i < oper_subs->sub_count); ++i) {
        ...
        /* parse data parent */
        if (lyd_parse_data_mem(conn->ly_ctx, oper_sub->sub_shm.addr + sizeof(sr_sub_shm_t) + sr_strshmlen(request_xpath),
                LYD_LYB, LYD_PARSE_ONLY | LYD_PARSE_STRICT, 0, &parent)) {
    ...
    *err_code = 0;
    ...
    if (rpc_sub->tree_cb) {
        ...
        *err_code = rpc_sub->tree_cb(tmp_sess, rpc_sub->xpath, input_op, sr_ev2api(event), request_id, *output_op,
        ...
        /* prepare XPath */
        op_xpath = lyd_path(input_op, LYD_PATH_LOG, NULL, 0);
    ...
    for (i = 0; i < rpc_subs->sub_count; ++i) {
        ...
        if (sr_shmsub_rpc_listen_is_new_event(multi_sub_shm, rpc_sub)) {
            ...
            if (!input) {
                ...
                ly_in_new_memory(rpc_subs->sub_shm.addr + sizeof *multi_sub_shm, &in);
                if (lyd_parse_rpc(conn->ly_ctx, in, LYD_LYB, &input, NULL)) {
                    sr_errinfo_new_ly(&err_info, conn->ly_ctx);
                    SR_ERRINFO_INT(&err_info);
                    goto cleanup_rdunlock;
                }
...
sr_shmsub_notif_listen_process_module_events(struct modsub_notif_s *notif_subs, sr_conn_ctx_t *conn)
{
    ...
    /* parse notification */
    ly_in_new_memory(notif_subs->sub_shm.addr + sizeof *multi_sub_shm + sizeof notif_ts, &in);
    if (lyd_parse_notif(conn->ly_ctx, in, LYD_LYB, &notif, NULL)) {
        sr_errinfo_new_ly(&err_info, conn->ly_ctx);
        SR_ERRINFO_INT(&err_info);
        goto cleanup_rdunlock;
    }
