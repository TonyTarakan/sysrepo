...
sr_modinfo_edit_apply(struct sr_mod_info_s *mod_info, const struct lyd_node *edit, int create_diff)
{
    ...
    LY_LIST_FOR(edit, node) {
        if (!strcmp(lyd_owner_module(node)->name, SR_YANG_MOD)) {
            str = lyd_path(node, LYD_PATH_LOG, NULL, 0);
...
sr_modinfo_diff_merge(struct sr_mod_info_s *mod_info, const struct lyd_node *new_diff)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & MOD_INFO_REQ) {
            ...
            if (lyd_diff_merge_module(&mod_info->diff, new_diff, mod->ly_mod,
                    mod_info->ds == SR_DS_OPERATIONAL ? sr_lyd_diff_merge_cb : NULL, mod_info->conn)) {
    ...
    *oper_data = NULL;
    ...
    if (parent) {
        ...
        if (request_xpath) {
            ...
            parent_path = lyd_path(last_parent, LYD_PATH_LOG, NULL, 0);
...
sr_modinfo_module_srmon_module(sr_main_shm_t *main_shm, char *ext_shm_addr, sr_mod_t *shm_mod, struct lyd_node *sr_state)
{
    ...
    /* module */
    SR_CHECK_LY_RET(lyd_new_inner(sr_state, NULL, "module", &sr_mod), ly_ctx, err_info);

    /* name */
    SR_CHECK_LY_RET(lyd_new_term(sr_mod, NULL, "name", ext_shm_addr + shm_mod->name, NULL), ly_ctx, err_info);

    /* subscriptions, make implicit */
    SR_CHECK_LY_RET(lyd_new_inner(sr_mod, NULL, "subscriptions", &sr_subs), ly_ctx, err_info);
    ...
    for (ds = 0; ds < SR_DS_COUNT; ++ds) {
        ...
        for (i = 0; i < shm_mod->change_sub[ds].sub_count; ++i) {
            /* change-sub */
            SR_CHECK_LY_RET(lyd_new_inner(sr_subs, NULL, "change-sub", &sr_sub), ly_ctx, err_info);

            /* datastore */
            SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "datastore", sr_ds2ident(ds), NULL), ly_ctx, err_info);

            /* xpath */
            if (change_sub[i].xpath) {
                SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "xpath", ext_shm_addr + change_sub[i].xpath, NULL),
                        ly_ctx, err_info);
            }

            /* priority */
            sprintf(buf, "%"PRIu32, change_sub[i].priority);
            SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "priority", buf, NULL), ly_ctx, err_info);
            ...
            SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "pid", buf, NULL), ly_ctx, err_info);
        }
    }
    ...
    for (i = 0; i < shm_mod->oper_sub_count; ++i) {
        /* operational-sub */
        SR_CHECK_LY_RET(lyd_new_inner(sr_subs, NULL, "operational-sub", &sr_sub), ly_ctx, err_info);

        /* xpath */
        SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "xpath", ext_shm_addr + oper_sub[i].xpath, NULL), ly_ctx, err_info);
    ...
    for (i = 0; i < shm_mod->notif_sub_count; ++i) {
        ...
        sprintf(buf, "%"PRIu32, pid);
        SR_CHECK_LY_RET(lyd_new_term(sr_subs, NULL, "notification-sub", buf, NULL), ly_ctx, err_info);
    }
...
sr_modinfo_module_srmon_rpc(sr_main_shm_t *main_shm, char *ext_shm_addr, sr_rpc_t *shm_rpc, struct lyd_node *sr_state)
{
    ...
    /* rpc */
    SR_CHECK_LY_RET(lyd_new_inner(sr_state, NULL, "rpc", &sr_rpc), ly_ctx, err_info);

    /* path */
    SR_CHECK_LY_RET(lyd_new_term(sr_rpc, NULL, "path", ext_shm_addr + shm_rpc->op_path, NULL), ly_ctx, err_info);

    rpc_sub = (sr_rpc_sub_t *)(ext_shm_addr + shm_rpc->subs);
    for (i = 0; i < shm_rpc->sub_count; ++i) {
        /* rpc-sub */
        SR_CHECK_LY_RET(lyd_new_inner(sr_rpc, NULL, "rpc-sub", &sr_sub), ly_ctx, err_info);

        /* xpath */
        SR_CHECK_LY_RET(lyd_new_term(sr_sub, NULL, "xpath", ext_shm_addr + rpc_sub[i].xpath, NULL), ly_ctx, err_info);
        ...
        struct lyd_node *sr_state)
{
    ...
    /* connection */
    SR_CHECK_LY_RET(lyd_new_inner(sr_state, NULL, "connection", &sr_conn), ly_ctx, err_info);

    /* pid */
    sprintf(buf, "%ld", (long)shm_conn->pid);
    SR_CHECK_LY_RET(lyd_new_term(sr_conn, NULL, "pid", buf, NULL), ly_ctx, err_info);
    ...
    if (shm_conn->main_lock.mode) {
        ...
        SR_CHECK_LY_RET(lyd_new_term(sr_conn, NULL, "main-lock", buf, NULL), ly_ctx, err_info);
    }
    ...
    for (i = 0; i < main_shm->mod_count; ++i) {
        for (ds = 0; ds < SR_DS_COUNT; ++ds) {
            ...
            /* module-lock */
            SR_CHECK_LY_RET(lyd_new_inner(sr_conn, NULL, "module-lock", &sr_modlock), ly_ctx, err_info);

            /* name */
            SR_CHECK_LY_RET(lyd_new_term(sr_modlock, NULL, "name", ext_shm_addr + shm_mod[i].name, NULL), ly_ctx, err_info);

            /* datastore */
            SR_CHECK_LY_RET(lyd_new_term(sr_modlock, NULL, "datastore", sr_ds2ident(ds), NULL), ly_ctx, err_info);
            ...
            SR_CHECK_LY_RET(lyd_new_term(sr_modlock, NULL, "lock", buf, NULL), ly_ctx, err_info);
        }
    }
...
sr_modinfo_module_data_load_srmon(struct sr_mod_info_s *mod_info)
{
    ...
    /* main container */
    SR_CHECK_LY_GOTO(lyd_new_inner(NULL, ly_mod, "sysrepo-state", &mod_data), mod_info->conn->ly_ctx, err_info, cleanup);
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & mod_state) {
            ...
            if (diff) {
                ...
                if (lyd_diff_merge_all(&mod_info->diff, diff)) {
                    sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
                    goto cleanup;
                }
...
sr_modinfo_add_np_cont(struct sr_mod_info_s *mod_info)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        case MOD_INFO_REQ:
            ...
            if (lyd_new_implicit_module(&mod_info->data, mod->ly_mod, 0, &d)) {
                sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
                return err_info;
            }

            /* merge diff */
            if (lyd_diff_merge_all(&mod_info->diff, d)) {
                sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
                return err_info;
            }
...
sr_modinfo_op_validate(struct sr_mod_info_s *mod_info, struct lyd_node *op, int output)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        case MOD_INFO_REQ:
            ...
            parent_xpath = lyd_path(lyd_parent(op), LYD_PATH_LOG, NULL, 0);
...
sr_modinfo_generate_config_change_notif(struct sr_mod_info_s *mod_info, sr_session_ctx_t *session)
{
    ...
    if (lyd_new_inner(notif, NULL, "changed-by", &root)) {
        sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
        goto cleanup;
    }

    /* changed-by username */
    if (lyd_new_term(root, NULL, "username", session->sid.user, NULL)) {
        sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
        goto cleanup;
    }
    ...
    if (lyd_new_term(root, NULL, "session-id", nc_str, NULL)) {
        sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
        goto cleanup;
    }

    /* datastore */
    if (lyd_new_term(notif, NULL, "datastore", sr_ds2str(mod_info->ds), NULL)) {
        sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
        goto cleanup;
    }
    ...
    while (!(err_info = sr_diff_set_getnext(set, &idx, &elem, &op)) && elem) {
        /* edit (list instance) */
        if (lyd_new_inner(notif, NULL, "edit", &root)) {
            sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
            goto cleanup;
        }

        /* edit target */
        xpath = lyd_path(elem, LYD_PATH_LOG, NULL, 0);
        if (!xpath) {
            sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
            goto cleanup;
        }
        lyrc = lyd_new_term(root, NULL, "target", xpath, NULL);
        free(xpath);
        if (lyrc) {
            sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
            goto cleanup;
        ...
        if (lyd_new_term(root, NULL, "operation", op_enum, NULL)) {
            sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
            goto cleanup;
        }
    }
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & MOD_INFO_CHANGED) {
            if (mod_info->ds == SR_DS_OPERATIONAL) {
                ...
                if (lyd_diff_merge_module(&diff, mod_info->diff, mod->ly_mod, sr_lyd_diff_merge_cb, mod_info->conn)) {
                    sr_errinfo_new_ly(&err_info, mod_info->conn->ly_ctx);
                    goto cleanup;
                }
